...
Borrowed from https://github.com/desihub/redrock/blob/main/py/redrock/plotspec.py
Modified by Noah Franz
"""
...
class PlotSpec(object):
    def __init__(self, targets, templates, zscan, zfit, maxZtoPlot=None, chi2range=None, nBestFits=None, truth=None, archetypes=False):
        """TODO: document
        """
        ...
        self.maxZ = maxZtoPlot
        self.chi2range = chi2range
        self.N = nBestFits
        ...
        plt.ion()
        self.plot()
        self._fig.savefig('rrplot-chi2.png', transparent=False, bbox_inches='tight')
        plt.show( block=True )
    ...
    def plot(self, keepzoom=False):
        ...
        where = np.where(zfit['z'] <= self.maxZ)[0]
        
        if self.N is None:
            self.N = len(zfit['z'])
        
        self._ax1.plot(zfit['z'][where][:self.N], zfit['chi2'][where][:self.N], 'r.', label='_none_')
        for ii, row in enumerate(zfit):
            if row['z'] <= self.maxZ and ii<self.N:
        ...
        if self.chi2range:
            self._ax1.set_ylim(self.chi2range[0], self.chi2range[1])
        ...
        self._ax2.set_ylabel('Flux [$10^{-17} erg\ s^{-1}\ cm^{-2}\ \AA^{-1}$]')
